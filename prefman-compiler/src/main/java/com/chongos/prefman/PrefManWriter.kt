...
internal class PrefManWriter(private val prefName: String,
    ...
    private fun createGetInstanceMethod(): List<MethodSpec> {
        ...
        methodSpecs.add(MethodSpec.methodBuilder("getManager")
                ...
                .addStatement("return getManager(context, prefName, mode, new \$T())", prefClass)
                .build()
        )
        methodSpecs.add(MethodSpec.methodBuilder("getManager")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                ...
                .addStatement("return getManager(context, \$S, \$L, customDefault != null ? customDefault : new \$T())", prefName, mode, prefClass)
                .build()
        )
        methodSpecs.add(MethodSpec.methodBuilder("getManager")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                ...
                .addStatement("\$N = new \$T()",
                        ...
                        manClass))
                .endControlFlow()
                .addStatement("\$T manager = (\$T) \$N.get(\$N)", manClass, manClass,
                        VAR_INSTANCE_MAP, "prefName")
